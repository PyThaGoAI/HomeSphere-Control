          ...
          <div className="p-6 bg-cosmic-blue/30 backdrop-blur-md border border-cosmic-teal/20 rounded-xl">
            ...
            <div className="space-y-4">
              {["Living Room", "Kitchen", "Bedroom", "Bathroom", "Office"].map((room) => (
                <div key={room} className="flex items-center justify-between p-3 bg-cosmic-blue/50 rounded-lg">
                  <span>{room}</span>
                  <div className="flex items-center">
                    <span className="mr-3 text-sm">Brightness</span>
                    <input 
                      type="range" 
                      className="w-32 h-1 bg-white/10 rounded-md appearance-none [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-cosmic-amber"
                    />
          ...
          <div className="p-6 bg-cosmic-blue/30 backdrop-blur-md border border-cosmic-teal/20 rounded-xl">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {["Movie Night", "Reading", "Dinner", "Party", "Night Mode", "Morning"].map((scene) => (
                <button key={scene} className="p-4 bg-cosmic-blue/50 rounded-lg text-center hover:bg-cosmic-blue/70 transition-colors">
                  <div className="w-12 h-12 mx-auto mb-2 rounded-full bg-cosmic-blue flex items-center justify-center">
                    <div className="w-8 h-8 rounded-full bg-cosmic-amber opacity-70"></div>
                  </div>
